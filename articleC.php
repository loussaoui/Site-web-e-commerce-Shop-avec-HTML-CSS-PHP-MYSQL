...
<tr><td>Nom</td><td>Src </td><td>Prix </td><td>Qauntité</td><td>Categorie</td></tr>					 ' ;
					 
					 while($ligne=mysqli_fetch_array($run)){
						 echo "<tr>" ;
						 
							 echo '<td> '.$ligne[0].'</td><td> '.$ligne[1].'</td><td> '.$ligne[2].'</td><td> '.$ligne[3].'</td><td> '.$ligne[4].'</td>' ;
...
if(isset($_POST['suprimer'])){
	...
	$sql = 'DELETE FROM `produits` WHERE nom="'.$_POST['nom'].'" ' ;
     $run=mysqli_query($lien,$sql);
